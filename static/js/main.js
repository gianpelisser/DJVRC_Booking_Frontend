...
function colorIndex(name) {
  const str = name || '?';
  return (str.charCodeAt(0) + (str.charCodeAt(1) || 0)) % 5;
}
...
document.querySelectorAll('[data-initials]').forEach(el => {
  ...
  el.style.background = AVATAR_COLORS[colorIndex(name)];
  el.style.display = 'flex';
  el.style.alignItems = 'center';
  el.style.justifyContent = 'center';
  el.style.color = '#fff';
  el.style.fontFamily = 'var(--font-d)';
});

// Banner color
document.querySelectorAll('[data-banner]').forEach(el => {
  const name = el.dataset.banner || '';
  el.style.background = BANNER_COLORS[colorIndex(name)];
});
